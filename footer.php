<?php
?>

<script src="https://kit.fontawesome.com/96a3a7d865.js" crossorigin="anonymous"></script>

<footer class="footer-downer bg-wine bottom-100 text-champagne mt-5 pb-5 border-champagne" style="background-image: linear-gradient(to top, #333333, #642A38);">
    <div class="container">
        <div class="row">
            <div class="col-12 col-lg-3 mt-4">
                <p class="h2 mb-4 fw-bolder text-center border-bottom border-3 pb-3">Vinkompagni</p>
                <p class="mb-1">Middelfart 2, 4200 Slagelse</p>
                <p class="mb-1">Danmark</p>
                <p class="mb-1">+45 58 56 14 00</p>
                <p class="mb-1">Email: ordre@slagelsevinkompagni.dk</p>
                <p class="mb-1">Cvr. nummer: 2723 4720</p>
            </div>
            <div class="col-12 col-lg-3 mt-4">
                <p class="h2 mb-4 fw-bolder text-center border-bottom border-3 pb-3">Information</p>
                <p class="mb-1">Om os</p>
                <p class="mb-1">Bestilling og levering</p>
                <p class="mb-1">Kontakt os</p>
                <p class="mb-1">Hjælp</p>
            </div>
            <div class="col-12 col-lg-3 mt-4">
                <p class="h2 mb-4 fw-bolder text-center border-bottom border-3 pb-3">Åbningstider</p>
                <div class="row text-center">
                    <div class="col-3">
                        Mandag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-17</p>
                    </div>
                    <div class="col-3">
                        Tirsdag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-17</p>
                    </div>
                    <div class="col-3">
                        Onsdag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-17</p>
                    </div>
                    <div class="col-3">
                        Torsdag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-17</p>
                    </div>
                    <div class="col-3">
                        Fredag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-17</p>
                    </div>
                    <div class="col-3">
                        Lørdag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-14</p>
                    </div>
                    <div class="col-3">
                        Søndag
                    </div>
                    <div class="col-6 border-bottom mb-1"></div>
                    <div class="col-3">
                        <p class="m-0">10-14</p>
                    </div>
                </div>
            </div>
            <div class="col-12 col-lg-3 mt-4">
                <p class="h2 mb-4 fw-bolder text-center border-bottom border-3 pb-3 fs-2">SoMe & Sponsorer</p>
                <div class="d-flex row-cols-4 text-center" style="font-size: 30px;">
                    <a><i class="fa-brands fa-facebook"></i></a>
                    <a><i class="fa-brands fa-instagram "></i></a>
                    <a><i class="fa-brands fa-linkedin"></i></a>
                    <a><i class="fa-brands fa-youtube"></i></a>
                </div>
            </div>
            <div class="col-12 mt-5 text-center" style="font-size: 25px;">
                <p class="fs-4 mb-2 fw-light"><i class="fa-regular fa-credit-card"></i> Betalings muligheder</p>
                <i class="ms-2 me-2 fa-brands fa-cc-visa"></i>
                <i class="ms-2 me-2 fa-brands fa-cc-mastercard"></i>
                <i class="ms-2 me-2 fa-brands fa-cc-paypal"></i>
                <i class="ms-2 me-2 fa-brands fa-cc-apple-pay"></i>
                <i class="ms-2 me-2 fa-brands fa-google-pay"></i>
            </div>
            <div class="col-12 text-center mt-5 mb-5">
                <p><i class="fa-regular fa-copyright"></i> Copyright 2025</p>
            </div>
        </div>
    </div>
</footer>
